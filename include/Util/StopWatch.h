 ...
 * class directly: `CoarseStopWatch` and `StopWatch`.
 *
 * Arguments:
 ...
 * Example 1:
 *
 *  CoarseStopWatch<std::seconds> watch;
 *  do_something();
...
struct custom_stop_watch {
    ...
    static_assert(
        std::ratio_less_equal<typename clock_type::duration::period,
     ...
     * that, in the rare occasions it is needed, one does not have to
     * reimplement
    ...
    duration elapsed() const {
        return std::chrono::duration_cast<duration>(clock_type::now() -
                                                    checkpoint_);
    }
     ...
     *  do_something();
     *
     *  std::cout << "has the TTL expired? " std::boolalpha<<
     * watch.elapsed(ttl);
     *
     * @author: Marcelo Juchem <marcelo@fb.com>
     */
    ...
    duration lap() {
        ...
        return std::chrono::duration_cast<duration>(checkpoint_ -
                                                    lastCheckpoint);
    }
...
using CoarseStopWatch = custom_stop_watch<monotonic_coarse_clock, Duration>;
...
using StopWatch = custom_stop_watch<monotonic_clock, Duration>;
}