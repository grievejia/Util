...
public:
    ...
    // Trying to use the assignment operator to assign a nn<PtrType> to a
    // PtrType using the
    ...
    // Can't convert to bool (that would be silly). The explicit delete results
    // in
    // "value of type 'nn<...>' is not contextually convertible to 'bool'",
    // rather than
    // "no viable conversion", which is a bit more clear.
    operator bool() const = delete;

    // Explicitly deleted constructors. These help produce clearer error
    // messages, as trying
    ...
    nn&
    operator=(PtrType) = delete; // must use NN_CHECK_ASSERT or NN_CHECK_THROW
    ...
    // writing
    // "base_ptr = derived_ptr;" will run the type-converting constructor
    // followed by the
